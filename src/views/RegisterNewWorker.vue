              ...
              <div class="nrcContainer col-12 col-md-7 col-lg-3">
                ...
                <div class="nrcForm d-flex">
                  <select v-model="stateId" @change="getStateid(stateId.id)">
                    <option
                      v-for="state in getAllStates"
                      :key="state.id"
                      :value="{ id: state.id, no: state.number.en }"
                    >
                      {{ state.number.en }}
                    </option>
                  </select>
...
export default {
  ...
  setup() {
    ...
    // get NRC Data
    let { getAllStates, getStateid, getAlltownships, getAlltypes } = nrcdata(
      stateId.id
    );
